...
The published implementation uses UUPS upgrade authorization. Its source requires unanimous approval from the three configured voter addresses before an implementation upgrade or ownership change can be executed. The ERC-20 owner has no independent upgrade authority. Approval state is scoped to the proposed action and consumed when the action executes.
